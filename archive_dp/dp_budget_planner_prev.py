...
def build_time_features(depth, base_filters, batch_size, params, steps_per_epoch):
    # MUST match train_time_ann_deploy.py / predict_perf_final.py feature transforms:
    # [depth, base_filters, log2(batch_size), log10(params), log10(steps_per_epoch)]
    x = np.zeros((1, 5), dtype=np.float32)
    x[0, 0] = float(depth)
    x[0, 1] = float(base_filters)
    x[0, 2] = math.log2(float(batch_size))
    x[0, 3] = math.log10(float(params))
    x[0, 4] = math.log10(float(steps_per_epoch))
    return x
...
def predict_time_per_epoch_sec(model, depth, base_filters, batch_size, params, steps_per_epoch):
    x = build_time_features(depth, base_filters, batch_size, params, steps_per_epoch)
    pred_log_time = float(model.predict(x, verbose=0).reshape(-1)[0])
    t = math.exp(pred_log_time)
