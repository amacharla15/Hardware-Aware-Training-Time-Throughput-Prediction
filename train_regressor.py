...
def train_target(rows, target_key, prefix):
    ...
    save_scatter(y_test, pred_lr, f"{target_key} LinearRegression", f"results/{prefix}_linear_scatter.png")
    ...
    save_scatter(y_test, pred_ann, f"{target_key} ANN", f"results/{prefix}_ann_scatter.png")
    model.save(f"results/{prefix}_ann_model.keras")


def main():
    os.makedirs("results", exist_ok=True)
    rows = load_rows("runs.csv")
    print("rows:", len(rows))
    train_target(rows, "avg_time_sec", "time")
    train_target(rows, "images_per_sec", "throughput")
